...
export default function App() {
  ...
  // iframeの中にいるか。埋め込み中は「問題演習に戻る」を出さない
  // （フレーム内で本体アプリに遷移してしまい、狭い枠に閉じ込められた行き止まりになるため）
  ...
  const questions = useMemo<Question[]>(() => {
    ...
    const ids =
      state.randomIds && state.randomIds.length > 0
        ? state.randomIds
        : sampleOrder
          ? sampleOrder === "all"
            ? SAMPLE_ORDER_IDS
            : SAMPLE_ORDER_BY_YEAR[sampleOrder]
          : null;
  ...
  /** サンプル順の切替。並びが変わっても「いま見ている問題」を保つ */
  const changeSampleOrder = (next: string | null) => {
    const currentId = questions[state.currentIndex]?.id;
    setSampleOrder(next);
    if (next) lsSet(SAMPLE_ORDER_KEY, next);
    else lsDel(SAMPLE_ORDER_KEY);
    const nextIds = !next
      ? allQuestions.map((q) => q.id)
      : next === "all"
        ? SAMPLE_ORDER_IDS
        : SAMPLE_ORDER_BY_YEAR[next] ?? [];
    const idx = currentId ? nextIds.indexOf(currentId) : -1;
    setState((prev) => ({ ...prev, currentIndex: idx >= 0 ? idx : 0 }));
  };
  ...
  // テキストヒントを開いている問題のid（問題を移ると自動的に閉じる）
  const [hintTextShownFor, setHintTextShownFor] = useState<string | null>(null);
  ...
  const [studied, setStudied] = useState<Record<string, 1>>(() => {
    try {
      const v = JSON.parse(lsGet(STUDIED_KEY) || "{}");
  ...
  const hintTextDisplay = currentQuestion?.hintText?.trim() ?? "";
  /** 表つきのヒント（hintText の代わり）。ヒントボタンの表示条件にも使う */
  const hintBlocksDisplay = currentQuestion?.hintBlocks ?? [];
  const hasHintText = hintTextDisplay !== "" || hintBlocksDisplay.length > 0;
  ...
  /** 学習済みを使うのは通常の問題演習だけ。模試・埋込・ランダム出題では出さない */
  const studiedEnabled = !isMogi && !embed && !state.randomIds;
  /** 学習済みフラグの手動トグル。埋め込み・模試では使わない */
  const toggleStudied = () => {
    setStudied((prev) => {
      const next = { ...prev };
      if (next[currentQuestion.id]) delete next[currentQuestion.id];
      else next[currentQuestion.id] = 1;
      lsSet(STUDIED_KEY, JSON.stringify(next));
      return next;
    });
  };
  ...
  const gradeExam = () => {
    ...
    setState((prev) => ({
      ...prev,
      answers: {},
      // 問題演習では「あとで見返す」は学習の記録なので採点しても残す。
      // 模試とランダム出題はその場限りの出題なのでクリアする
      reviewFlags: isMogi || prev.randomIds ? {} : prev.reviewFlags,
      currentIndex: 0
    }));
  };

  const handleFinish = () => {
    // 問題演習では回答だけがクリアされる。学習の記録が消えないことを明示しておく
    const msg = studiedEnabled
      ? "終了して採点しますか？\n（「あとで見返す」と「学習済み」は、クリアされずに残ります）"
      : "終了して採点しますか？";
    ...
    <div className={embed ? "app app--embed" : "app"}>
      ...
      <main className="layout" style={paneStyle}>
        ...
        <section className={`pane choices ${isMobile && mobilePane !== "choices" ? "mobile-hidden" : ""}`}>
          <div className="choices-header">
            ...
            {state.perQuestionGrading && !isMogi && (
              <>
                ...
                {(hintVideoUrlDisplay || hasHintText) && (
                  <button
                    type="button"
                    className="outline"
                    aria-expanded={!hintVideoUrlDisplay ? hintTextShownFor === currentQuestion.id : undefined}
                    onClick={() => {
                      // 動画優先: hintVideoUrl があれば動画を開く。無いときだけテキストを開閉する
                      if (hintVideoUrlDisplay) {
                        window.open(hintVideoUrlDisplay, "_blank", "noopener,noreferrer");
                      } else {
                        setHintTextShownFor((prev) => (prev === currentQuestion.id ? null : currentQuestion.id));
                      }
                    }}
                  >
                    ヒント
            ...
            {((state.perQuestionGrading && !isMogi) || (isMogi && reviewMode)) && (
              <>
                ...
                <button
                  ...
                  onClick={() => {
                    // 進捗集計: 問題演習で「解説へ」を開いた記録を送る（模試・埋込は対象外）
                    if (!isMogi && !embed) {
                      const sel = state.answers[currentQuestion.id];
                      postLessonOpen({
                        clientId: getClientId(),
                        qId: currentQuestion.id,
                        qNumber: currentQuestion.number,
                        ok: sel ? (sel === currentQuestion.correctChoiceId ? 1 : 0) : null,
                        device: detectDevice()
                      });
                    }
          ...
          {!hintVideoUrlDisplay && hasHintText && hintTextShownFor === currentQuestion.id && (
            <div className="hint-text-panel">
              {hintTextDisplay ? <p className="hint-text-line">{hintTextDisplay}</p> : null}
              {hintBlocksDisplay.map((block, idx) => renderBodyBlock(block, idx))}
            </div>
          )}
        ...
        <div className={singleQuestion ? "bottom-bar-inner is-single" : "bottom-bar-inner"}>
          ...
          {studiedEnabled && (
            <button
              ...
              className={`outline studied-btn ${studied[currentQuestion.id] ? "on" : ""}`}
              aria-pressed={Boolean(studied[currentQuestion.id])}
            >
        ...
        <div className="overlay">
          <div className="overlay-content">
            <div className="overlay-header">
              <h3>設定</h3>
              <button className="outline" onClick={() => setShowSettings(false)}>
                閉じる
              </button>
            </div>
            <div className="mode-options" style={{ textAlign: "left" }}>
              {!isMogi && (
                <>
                  <label>
                    <input
                      type="checkbox"
                      checked={state.perQuestionGrading}
                      onChange={(e) =>
                        setState((prev) => ({ ...prev, perQuestionGrading: e.target.checked }))
                      }
                    />{" "}
                    問題ごとに採点
                  </label>
                  <label>
                    <input
                      type="checkbox"
                      checked={state.perQuestionTimer}
                      onChange={(e) =>
                        setState((prev) => {
                          const next = { ...prev, perQuestionTimer: e.target.checked };
                          // 途中でONにした場合、今の問題の残り時間が無いとタイマーが動かないので初期化する
                          const qid = questions[prev.currentIndex]?.id;
                          if (e.target.checked && qid && prev.perQuestionRemainingSeconds[qid] === undefined) {
                            next.perQuestionRemainingSeconds = {
                              ...prev.perQuestionRemainingSeconds,
                              [qid]: PER_QUESTION_TIME
                            };
                          }
                          return next;
                        })
                  ...
                  {state.perQuestionTimer && (
                    <label className="mode-option-indent">
                      <input
                        type="checkbox"
                        checked={state.perQuestionTimerAlert}
                        onChange={(e) =>
                          setState((prev) => ({ ...prev, perQuestionTimerAlert: e.target.checked }))
                        }
                      />{" "}
                      5分経過をウィンドウで知らせる
                    </label>
                  ...
                  <label>
                    <input
                      type="checkbox"
                      checked={state.showQuestionNumber}
                      onChange={(e) =>
                        setState((prev) => ({ ...prev, showQuestionNumber: e.target.checked }))
                      }
                    />{" "}
                    問題番号を表示
                  </label>
              ...
              <label>
                <input
                  type="checkbox"
                  checked={state.instructorMode}
                  onChange={(e) =>
                    setState((prev) => ({ ...prev, instructorMode: e.target.checked }))
                  }
                />{" "}
                講師モード
              </label>
        ...
        <div className="overlay">
          <div className="overlay-content overlay-content--question-list">
            {(() => {
              ...
              const cell = (q: (typeof questions)[number], idx: number, label?: string) => {
                const answered = Boolean(state.answers[q.id]);
                const review = Boolean(state.reviewFlags[q.id]);
                const done = studiedEnabled && Boolean(studied[q.id]);
                return (
                  <button
                    key={q.id}
                    onClick={() => jumpTo(idx)}
                ...
                return (
                  <>
                    ...
                    <div className="question-list-scroll" role="region" aria-label="問題番号一覧">
                      <div className="grid">{questions.map((q, idx) => cell(q, idx))}</div>
                    </div>
                  </>
                );
              }
                      ...
                      {WEEKS.map((w) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            role="tab"
                            aria-selected={w.week === activeWeek}
                            className={`tab ${w.week === activeWeek ? "active" : ""} ${
                              items.length > 0 && done === items.length ? "tab--done" : ""
                            }`}
...
function ModePicker({ isMogi = false, isTrial = false, onStart, onTrialLock, mogiLocked = false, onCheckUnlock }: ModePickerProps) {
  ...
  return (
    <div className="mode-picker">
      ...
      {/* 共有NGの注意。モード選択を開くたびに毎回目に入るよう、常時表示する */}
      <p className="share-note">※こちらのサイトは､共有NGです｡</p>
    </div>
  );
}
