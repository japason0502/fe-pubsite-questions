...
.rr-card{display:block;width:100%;max-width:560px;margin:14px auto 0;border-radius:12px;box-shadow:0 6px 20px rgba(15,23,42,.14)}
...
.rr-fold summary{cursor:pointer;color:var(--rr-muted);font-size:.85rem}
.rr-fold[open] summary{margin-bottom:2px}
 ...
 * YouTube のコメント欄には画像を貼れないので、代わりに採点結果の文をコピーしておく。
 * 遷移（target="_blank"）は止めないよう、await せずに投げるだけにする。
 */
function copyThenGo(shareText: string) {
  try {
