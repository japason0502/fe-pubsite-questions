...
export type ZoneReport = {
  ...
  numbers: number[];      // 対象の問番号（レポートの集計用）
  got: number;            // 正答数
  total: number;          // 問数
  quota: number;          // ノルマ
  met: boolean;           // ノルマ達成か
...
const CONFIG: Record<string, SetConfig> = {
  ...
  // 模試②。ゾーンの並びは①と同じだが、どの問がどのゾーンかは①と違う
  "2": {
    basicsTrace: [1, 4],
    basicsReading: [2, 3, 5],
    fieldByNumber: {
...
const UNTOUCHED_SEC = 45;  // 不正解かつこれ以下なら「手つかず」（ほぼ当てずっぽうで答えている）
const RUSH_AVG_SEC = 180;  // 基礎ゾーン・情報セキュリティのミス問題の平均滞在がこれ未満なら「急ぎすぎ」
...
export function buildExamReport(set: string | null, rows: ReportRow[]): ExamReport | null {
  ...
  if (hasTime) {
    ...
    const untouched = rows.filter((r) => r.answered && !r.ok && r.sec <= UNTOUCHED_SEC).map((r) => r.n);
    if (untouched.length > 0) {
      time.push({
        tone: "warn",
        text: `${nums(untouched)}はほとんど手をつけずに答えています。時間切れなら、前半の時間配分を見直しましょう`
      });
    }
  }

  // 未回答は時間の計測有無に関わらず指摘する（未回答は0点）
  const unanswered = rows.filter((r) => !r.answered).map((r) => r.n);
  if (unanswered.length > 0) {
    time.push({
      tone: "warn",
      text: `${nums(unanswered)}が未回答です。分からなくても、何かしら選ぶ癖を付けましょう`
    });
  }

  /* ---------- ゾーンごとの達成状況＋コメント ---------- */

  /** ミス問題の平均滞在が短い＝手を動かさずに選んでいる（基礎ゾーンと情報セキュリティで使う） */
  const isRush = (missed: number[]): boolean => {
    const secs = missed.map((n) => by.get(n)!.sec).filter((s) => s > 0);
    const avg = secs.length > 0 ? secs.reduce((a, b) => a + b, 0) / secs.length : 0;
    return avg > 0 && avg < RUSH_AVG_SEC;
  };
  ...
  const zone = (name: string, ns: number[], quota: number, comments: ReportLine[]): ZoneReport => {
    const g = got(ns);
    return { name, range: nums(ns), numbers: ns, got: g, total: ns.length, quota, met: g >= quota, comments };
  };
  ...
  // 情報セキュリティ（基礎ゾーンと同じく「急ぎすぎ」を言い分ける）
